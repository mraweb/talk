...
class Empresa extends CriaPaginacao{
	...
	public function geraDadosIdEmpresa(){
		$sql = "
			...
			LEFT JOIN estados AS uf ON uf.id = e.estado_id
			WHERE e.dataExclusao IS NULL
			";
...
//------acoes------------------------------------------------------------------------------------------------------
	...
	if(isset($_POST["cadastrar"])){
		...
		if($cad->getDadosDuplicados($cnpj) >= 1){
			...
			$cad->setDados("'$status_id','$fantasia','$sintonia','$outorga_id','$prefixo','$razao','$cnpj','50','$estado_id','$cidade','$tel','$endereco','$bairro','$num','$cep','$diretorcomercial','$mailcomercial','$diretorartistico','$mailartistico','$msn','$site','$nome','$servico','$onde','$necessidade','$telefoneDiretorGeral','$telefoneDiretorArtistico','$obs','$dataCadastro'");
			$cad->insert();
	...
	if(isset($_POST["alterar"])){
		...
		$alt->setTable("empresa");//envio o nome da tabela
			...
			$alt->setFields("status_id='$status_id',fantasia='$fantasia',sintonia='$sintonia',outorga_id='$outorga_id',prefixo='$prefixo',razao='$razao',cnpj='$cnpj',pais_id='50',estado_id='$estado_id',cidade='$cidade',tel='$tel',endereco='$endereco',
