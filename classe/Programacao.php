...
class Programacao extends CriaPaginacao{
	...
	public function getListProgramacao(){ 
		...
		if($qtRegistros > 0){
			while($dados = self::resultsAll($this->qr)){
				...
				$link = URL.'programas/'.criarSlug($dados['nomeServico']).'/'.criarSlug($dados['nomeCategoriaServico']).'/'.criarSlug($dados['nomeProgramacao']);
	...
	public function getAcessoDiretoProgramacaoPorIds(){ 
		...
		*/
		...
		";
		if($this->idCategoriaServico)
		      $sql .=" AND p.categoria_servico_id = $this->idCategoriaServico";

		$sql .= "
		       AND p.status = 'A'
		       ORDER BY cs.nome, p.tipo_programacao, p.nome
		       LIMIT 1 
		";
