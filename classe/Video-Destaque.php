...
class VideoDestaque extends CriaPaginacao{
	...
	public function geraLisVideoDestaque(){ 
	...
	//lista videodestaque
	public function getListAllVideoDestaques(){ 
		...
		if($qtRegistros > 0){
			while($dados = self::results()){
				...
				echo'	            
		            <li '.$class.'><img src="'.$caminhoImagem.'" alt="'.$dados["nome"].'" width="199" height="116" /></li>
				';
				self::setContador($contador);
				$i++;
			}
	...
	//lista videodestaque pelo id
	public function geraDadosIdVideoDestaque(){
	...
	public function excluirArquivo(){ 
		...
		$sql = " UPDATE videodestaque SET imagem = '' "; 
		if($this->id)
...
//------acoes------------------------------------------------------------------------------------------------------
	...
	if(isset($_POST["cadastrar"])){
		...
		$cad->setTable("videodestaque");
		$cad->setFields("nome,imagem,dataCadastro");
		$cad->setDados("'$nome','$imagem','$dataCadastro'");
		$cad->insert();
		$idRegistro = $cad->getRetornaIdCadastro(); 
		$erro = base64_encode($cad->getStatus());
		
		$urlRedirecionamento = '../admin/?telas=video-destaque&msn='.$erro;
	...
	if(isset($_POST["alterar"])){
		...
		$alt->setTable("videodestaque");//envio o nome da tabela
		//enviando os atributos do banco
		$alt->setFields("nome='$nome', dataAlteracao='$dataAlteracao'");
		...
		$urlRedirecionamento = '../admin/?telas=video-destaque&idAlteracao='.$idAlteracao.'&acao=alterar&msn='.$erro;
