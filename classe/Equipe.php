...
class Equipe extends CriaPaginacao{
	...
	public function geraLisEquipe(){ 
	
		$sql = "
			SELECT id,nome,email,imagem,descricao
			FROM equipe
			...
			while($dados = self::results()){
				...
				echo'
					<tr '.$class.'>
		            	<td>'.$dados["nome"].'</td>
		                <td>'.$dados["email"].'</td>
		                <td class="tamanho1"><a href="?telas=novo-colaborador&idAlteracao='.$dados["id"].'&acao=alterar"><img src="img/bteditar.png" width="55" height="17" /></a></td>
	...
	public function geraDadosIdEquipe(){
	...
	//lista a equipe quem somos
	public function listAllEquipe(){ 
		...
		if($qtRegistros > 0){
			while($dados = self::results()){
				$imagem = '<img src="'.URL.'img/equipe/equipe-sem-imagem.png" alt="" width="120" height="120" class="left" />';
				$imagemLi = URL.'img/equipe/equipe-sem-imagem.png';
				if($dados['imagem']){
					$caminhoImagem = URL.'upload/arquivos/'.$dados['imagem'];
					$imagem = '<img src="'.$caminhoImagem.'" alt="'.$dados['nome'].'" width="120" height="120" class="left" />';
					$imagemLi = $caminhoImagem;
				}
					
				if($this->type == "detalhes"){
					echo'
						<section id="'.criarSlug($dados['nome']).'" class="tab_content-equipe">
		                	'.$imagem.'
		                    
		                    <div>
		                    	<h3>'.$dados['nome'].'</h3>
		                        
		                        <p>'.nl2br($dados['descricao']).'</p>
		                    </div>
		                </section>
					';
				}else{
					echo'
						<li><a href="#'.criarSlug($dados['nome']).'" title="'.$dados['nome'].'"><img src="'.$imagemLi.'" alt="'.$dados['nome'].'" width="96" height="96" /></a></li>
					';
				}
			}
		}
...
//------acoes------------------------------------------------------------------------------------------------------
	...
	if(isset($_POST["cadastrar"])){
		...
		$cad->setTable("equipe");
		$cad->setFields("nome,email,imagem,descricao,dataCadastro");
		$cad->setDados("'$nome','$email','$imagem','$descricao','$dataCadastro'");
		$cad->insert();
		$idRegistro = $cad->getRetornaIdCadastro(); 
		$erro = base64_encode($cad->getStatus());
		
		$urlRedirecionamento = '../admin/?telas=novo-colaborador&msn='.$erro;
	...
	if(isset($_POST["alterar"])){
		...
		$alt->setTable("equipe");//envio o nome da tabela
		//enviando os atributos do banco
		$alt->setFields("nome='$nome', email='$email',  descricao='$descricao', dataAlteracao='$dataAlteracao'");
		...
		$urlRedirecionamento = '../admin/?telas=novo-colaborador&idAlteracao='.$idAlteracao.'&acao=alterar&msn='.$erro;
