...
<div class="content">
    ...
    <form action="../classe/Noticia.php" method="post" id="formNoticia" method="post" enctype="multipart/form-data">
        <ul class="form1">
            ...
            <li>
                ...
                <input type="hidden" name="arquivo" id="add-banner" />
            ...
            <li>
                ...
                <span id='banner-imgs'>
                	<?php foreach($listagem->getBanners() as $banner):?>
    ...
    <div class="banners-cadastrados <?php if(!count($listagem->getBanners())):?>hide<?php endif?>">
		<div class="thumbs">
    	<?php foreach($listagem->getBanners() as $banner):?>
			<div class="thumb-container"><img src="<?php echo $banner['imagem']?>" /><input type='text' class='thumb-input' placeholder='link' value='<?php echo $banner['link']?>' data-id='<?php echo $banner['id']?>'><span class="excluir-banner" id="<?php echo $banner['imagem'].'|'.$banner['id']?>">EXCLUIR</span></div>
		<?php endforeach?>
		</div>
    </div>
</div>
