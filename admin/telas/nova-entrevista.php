...
include_once("../classe/Entrevista.php");
include_once("../funcoes/define.php");
	$listagem = new Entrevista();
	if($idAlteracao){
		$listagem->setId($idAlteracao);
		$listagem->geraDadosIdEntrevista();
	}
...
<div class="content">
	<p>ADICIONAR NOVA ENTREVISTA</p>
    
    <form action="../classe/Entrevista.php" method="post" id="formEmprevista" method="post" enctype="multipart/form-data">
        <ul class="form1">
            ...
            <li>
                ...
                <textarea cols="80" id="chamada" name="chamada" rows="10"><?php echo $listagem->getChamada();?></textarea>
                Restam <span id="contador_char"></span> caracteres a serem digitados.
            </li>
            
            <li>
                <label for="arquivo">Imagem:</label>
                <input type="file" name="arquivo_0" id="botao_enviar_arquivo" />
            </li>
            ...
            <li>
                <label for="descricao">Descrição:</label> Setar %%video%% e %%audio%%
                <textarea cols="80" id="editor1" name="descricao" rows="10"><?php echo $listagem->getDescricao();?></textarea>
            </li>
            
            <li>
                <label for="titulo">Vídeo:</label>
                <input type="text" name="video" id="video" value='<?php echo $listagem->getVideo();?>' />
            </li>
            
            <li>
                <label for="titulo">Áudio:</label>
                <input type="text" name="audio" id="audio" value='<?php echo $listagem->getAudio();?>' />
            </li>
	    ...
	    <ul class="galeria" id="galeria">
			...
			if($idAlteracao){
				$listagem->setId($idAlteracao);
				$listagem->geraImagensEntrevista();
			}
			?>
	    </ul>
    </form>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if(getImagem){
		    ...
		    var conteudo_imagem = '<img src="../upload/img_medium/'+getImagem+'" />'; 
	...
	$(function(){
		...
		var tamanhoSmall = "88";
		var tamanhoMedium = "166";
		var tamanhoLarge = "288";
		var pos_1 = 0;
		//-------------------------------------------------------------------------
		new AjaxUpload($('#botao_enviar_arquivo'), //botao que var permitir o usuário escolher o arquivo
		    {
			action: '../funcoes/uploadImgFull.php', //nome do script que vai tratar a requisição enviando o arquivo
			name: 'arquivo_'+pos_1+'', //nome do campo de arquivo do form que vai ser enviado, no php vai o arquivo vai ser acessado como $_FILES['arquivo']
			...
			onComplete: function(arquivo, resposta){ //arquivo é o nome do arquivo enviado, resposta é a resposta do servidor
				...
				if(resposta != "error" && resposta != "tamanho incorreto"){
				    ...
				    var conteudo_imagem = '<img src="../uploadTemp/img_medium/'+ resposta + '" />'; 
				    $('#conteudoImagemTemp').html(conteudo_imagem);
			...
			data: {
			    valor1 : tipoArquivo,
			    valor2 : acaoUpload,
			    tamanhoSmall: tamanhoSmall,
			    tamanhoMedium: tamanhoMedium,
			    tamanhoLarge: tamanhoLarge,
				posicao: pos_1
			}
		});
	...
	$(function(){
		$('#chamada').limit('140','#contador_char');
	});
</script>
